...
class TransactionReceipt:
    ...
    * Accessing methods / attributes that query debug_traceTransaction
    ...
    # @trace_property
    def trace(self, path = None) -> Optional[List]:
        if self._trace is None:
            self._expand_trace(path)
        return self._trace
    ...
    def _set_from_receipt(self, receipt: Dict) -> None:
        ...
        if receipt["contractAddress"] == "0x0000000000000000000000000000000000000000":
    ...
    def _get_trace(self, path = None) -> None:
        ...
        if path is None:
            raise Exception("the path of debug trace is none") from None
        ...
        self._raw_trace = trace = trace["structLogs"]
        if not trace:
            self._modified_state = False
            return
    ...
    def _expand_trace(self, path = None) -> None:
    ...
    def traceback(self, path = None) -> None:
        print(self._traceback_string(path) or "")

    # @trace_inspection
    def _traceback_string(self, path = None) -> str:
